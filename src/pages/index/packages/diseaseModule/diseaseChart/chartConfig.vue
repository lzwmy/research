    ...
    methods:{
      ...
      deleteModel(data) {
        this.$confirm('是否确定删除','提示',{
          ...
          // console.log('确定删除');
          this.customChartDelete(data.chartId).then(()=>{
            this.initPage();
          });
        }).catch(()=>{
          this.$notice("已取消删除");
      ...
      async modelCRFFromList() {
        ...
        try{
          ...
          let data = await that.$http.formSearchList(fromData);
          if(data.code == 0) {
      ...
      async getChartListModel() {
        ...
        try{
          ...
          let data = await that.$http.chartGetChartListNew(fromData);
          if(data.code == 0) {
      ...
      async saveCustomChart() {
        ...
        let formData = {
          ...
          "crfId": that.crf || "",
          "diseaseId":that.$route.query.id || "",
          "path":that.statistics || "",//统计指标路径
          "valueText": that.valueText || "",
          "comparePath":that.compareFormItemId || "",//对比指标路劲
          ...
          if(data.code == 0) {
            ...
            switch (data.data.chartType) {
              case "PIE":
                ...
                that.statistics = data.data.path;
                that.valueText = data.data.valueText;
                break;
              case "BAR":
                ...
                that.statistics = data.data.path;
                that.valueText = data.data.valueText;
                that.compareFormItemId = data.data.comparePath;
                that.compareValuePath = data.data.compareValueText
                break;
              case "LINE":
      ...
      async customChartDelete(value) {
        ...
        try{
          ...
          let data = await that.$http.chartCustomChartDeleteNew(fromData);
          if(data.code == 0) {
