...
export default {
    ...
    methods:{
        ...
        changeContent(item ,check) {
            if(check) {
                let exist = Array.from($('#content').children('span')).some((ele) =>{
                    return $(ele).attr('data-id') === item.treeId;
                })
                if(!exist) {
                    $('#content').append('<span data-id='+ item.treeId +' style="background:'+ this.colorArr[Math.floor(Math.random()*6)] + ';">'+ item.labelName +'</span>')
                }
            }else {
                $('#content span[data-id = '+ item.treeId+']').remove();
            }
        }
    },
}
