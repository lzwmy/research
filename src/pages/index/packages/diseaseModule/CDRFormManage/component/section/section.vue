<template>
    <div class="section_container">
      <div class="section_box">
        <div class="section_title">编辑小节</div>
        <div class="section_header_box">
          <div class="section_header_content">
            ...
            <div class="section_containEntry">
              <div class="containEntry_name">包含的条目</div>
            ...
            <div class="section_selectBtn">
              ...
              <div class="section_header_btn">
                ...
                <el-button @click="toJumpNewEntry">定义新条目</el-button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="section_body_box">
          ...
          <div class="footer_btn">
            ...
            <pagination :data="paginationData" @change="cgetPortionList"></pagination>
          </div>
        </div>
      </div>
    </div>
    ...
    export default {
      ...
      watch:{
        ...
        objType:function(value) {
          console.log('监听 父级传入参数',value);
          this.crfPortionModifyPreview(value.obj.id);
          this.cgetPortionList()
        }
      },
      methods:{
        //表格多选
        ...
        handleCurrentChange(value){
          ...
          this.crfGetPortionPage()
        },
        //返回 按钮
        returnPortion() {
          this.searchName = "";
          this.$emit("add-change",{type:'sectionList',clickType:"return",obj:{}})
        },
        //最新 条目删除
        tagClose(tag) {
        ...
        //跳转 定义新条目
        toJumpNewEntry() {
          // this.$emit("add-change",{type:'newEntry',clickType:'add',obj:{}})
          this.$emit("add-change",{type:'newEntry',clickType:'add',obj:{callback:'sectionList'}})
        },
        //删除
        clickProtion() {
        ...
        //获取编辑小节 条目列表
        async cgetPortionList(pageNo = this.pageNo, pageSize = this.pageSize){
          this.loading =  true;
          let that = this;
          let formData = {
            "formItemPageReq": {
              "author": "",
              "controlName":that.searchName || "",
              "controlType": "",
          ...
          try{
            let data = await that.$http.crfEntryList(that.$format(formData));
            console.log("小节列表 获取 条目列表")
            if(data.code===0){
              let obj = {};
              that.tableData = data.data.formItemsRspList;
              that.total = data.data.pageRsp.totalElements;
              obj.content = data.data.formItemsRspList;
        ...
        async crfPortionModifyPreview(value) {
          ...
          try{
            let data = await that.$http.crfPortionModifyPreview(that.$format(formData))
            console.log("小节编辑回显",data);
            if(data.code == '0') {
              that.from.portionName = data.data.portionName;
              that.from.displayPortionName = data.data.portionDisplayName;
              that.from.formType = data.data.displayIsVision.toString();
              that.IsWhether = data.data.portionIsAvailable.toString();
              that.id = data.data.id;
              that.formItemSimpleRspList = data.data.formItems;
              that.multipleSelection = data.data.formItems;
            }
            this.loading = false;
          }catch (error) {
        ...
        async crfEditPortionSave() {
          ...
          let formData = {
            ...
            "formItemIds": this.multipleSelection.map(item=>{
              return item.id
            }) || [],
            "id": that.id,
            "portionDisplayName": that.from.displayPortionName,
            "portionIsAvailable":parseInt(that.IsWhether),
            "portionName": that.from.portionName
          };
          try {
            let data = await that.$http.crfEditPortion(that.$format(formData))
        ...
        async crfDuplicateSave(value) {
          ...
          try{
            let data = await that.$http.crfDuplicateSave(that.$format(formData))
        ...
        async crfPortionDelete() {
          ...
          try {
            let data = await that.$http.crfPortionDelete(this.$format(formData));
            if(data.code==0) {
              that.$message({
                type:"success",
                message:data.msg
