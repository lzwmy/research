<template>
  <div class="add_report_container">
    <div class="section_box">
      <div class="section_title">编辑报告</div>
      <div class="section_header_box">
        <div class="section_header_content">
          <div class="section_termFilter">
            <div class="section_nameInput">
              <span>报告名称</span>
              <el-input placeholder="请输入报告名称"  v-model="from.portionName" :disabled="true" size="mini"></el-input>
            </div>
            <div class="section_nameInput">
              <span>显示名称</span>
            ...
            <div class="entry_box">
              <draggable v-model="multipleSelection">
                ...
                <el-tag
                  ...
                  :key="tag.formPageName"
                  closable
                  :disable-transitions="false"
                  @close="tagClose(tag,index)"
                  :type="tag.formPageName">
                  {{tag.formPageName}}
                </el-tag>
              </draggable>
            </div>
          ...
          <div class="section_selectBtn">
            <div class="section_header_btn">
              ...
              <el-button @click="clickDuplicateSave">保存为副本</el-button>
  ...
  export default {
    ...
    watch:{
      ...
      objType:function(value) {
        this.crfReportEditPreview(value.obj.id)
      }
    },
    methods: {
      //表格多选事件
      ...
      //返回
      returnPortionList() {
        this.$emit("add-change", {type: 'reportList', clickType: "return", obj: {}})
      },
      //定义新小节
      clickToJumpEntry() {
        // this.$emit("add-change",{type:'addSinglePage',clickType:'addReport',obj:{}})
      ...
      clickDuplicateSave() {
        console.log("报告保存副本")
        // this.crfReportDuplicate(this.id)
        this.crfAddPortionSaveNew();
      ...
      // 编辑回显
      async crfReportEditPreview(value) {
        let that = this;
        let formData = {
          formCrfId: value
        }
        try {
          let data = await that.$http.crfReportEditPreview(that.$format(formData))
          console.log("报告 编辑回显", data)
          if (data.code == 0) {
            that.id = data.data.id;
            that.from.portionName = data.data.crfName;
            that.from.displayName = data.data.crfDisplayName;
            that.from.access = data.data.crfType.toString();
            that.from.MedicalClassification = data.data.termCategoryNames;
            that.from.diseases=data.data.diseaseIds;
            that.from.description = data.data.crfDesc;
            that.from.available = data.data.crfIsAvailable.toString();
            that.multipleSelection = data.data.formPages
          ...
          that.$notice("获取编辑回显 参数失败");
          console.log(error)
        }
      },
      //删除
      async crfReportDelete() {
        let that = this;
        let formData = {
        ...
        try {
          ...
          if (data.code == 0) {
            ...
            this.$emit("add-change", {type: 'reportList', clickType: "reportList", obj: {}})
          }
        } catch (error) {
          that.$notice("删除失败");
          console.log(error)
      ...
      async crfReportDuplicate(value) {
        let that = this;
        let formData = {
          formCrfId:value
        }
        try{
          let data = await that.$http.crfReportDuplicate(that.$format(formData))
      ...
      async crfAddPortionSaveNew() {
        ...
        try {
          let data = await that.$http.crfReportDuplicate(formData);
          console.log("报告 新副本保存" , data)
          if(data.code == 0) {
            that.id = data.data;
            that.$message({
              type:"success",
    ...
    mounted() {
      ...
      this.getCategory();
      this.findDisease();
      this.crfReportEditPreview(this.objType.obj.id)
    }
  }
</script>
