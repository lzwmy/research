  ...
  <div class="entry_container" v-if="comShow">
    <div class="entry_search">
      <el-form :inline="true" :model="formInline">
        ...
        <el-form-item>
          ...
          <el-button @click="newEntry">新增条目</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="entry_content">
      ...
      >
        ...
        <el-table-column prop="controlName" label="控件名称"></el-table-column>
        <el-table-column prop="controlDisplayName" label="显示名称"></el-table-column>
        <el-table-column prop="controlType" label="显示类型" show-overflow-tooltip></el-table-column>
        <el-table-column prop="author" label="作者" show-overflow-tooltip></el-table-column>
        <el-table-column prop="reference" label="引用" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getFormItemList(pageNo = this.pageNo, pageSize = this.pageSize) {
      ...
      let formData = {
        formItemPageReq: {
          ...
          controlName: that.formInline.entryName||"",
          controlType: that.formInline.entryDisplayType || "",
          formType: ""
        },
        pageReq: {
          limit: pageSize,
    ...
    async batchDeletion() {
      ...
      let DelArray = that.multipleSelection.map(item=>{
        return item.id
      });
      let formData = {
        // list: that.delList
      ...
      try {
        let data = await that.$http.crfDelList(that.$format(formData));
        console.log(data);
        if (data.code == 0) {
          that.$message({
            type: "success",
