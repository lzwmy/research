<template>
    <div class="add_section_container">
      <div class="section_box">
        <div class="section_title">新建小节</div>
          ...
          <div class="section_header_content">
            ...
            <div class="section_selectBtn">
              <div class="section_header_btn">
                ...
                <el-button @click="clickToJumpEntry">定义新条目</el-button>
              </div>
            </div>
          </div>
        </div>
    ...
    export default {
      ...
      methods:{
        ...
        returnPortionList(){
          ...
          if(this.objType.obj.callback == 'breakSinglePage'){
            this.$emit("add-change",{type:'breakSinglePage',clickType:"breakSinglePage",obj:{callback:''}})
          }else{
        ...
        async crfAddPortionSave() {
          ...
          let formData = {
            ...
            "formItemIds": that.multipleSelection.map(item=>{
              return item.id
            })||[],
            "id": "",
            "portionDisplayName": that.from.displayName,
            "portionIsAvailable": that.from.isAvailable,
            "portionName": that.from.portionName
          }
          try {
            let data = await that.$http.crfAddPortionSave(that.$format(formData))
            console.log("新建小节 保存" , data)
            if(data.code == 0) {
              that.$message({
                type:"success",
                message:data.msg
