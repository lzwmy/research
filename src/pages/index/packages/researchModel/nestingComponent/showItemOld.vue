        ...
        methods:{
            ...
            onSelectAll() {
                ...
                this.findLeafItem().forEach( item =>{
                    $('#content').append('<span data-id='+ item.uniqueId +' style="background:'+ this.colorArr[Math.floor(Math.random()*6)] + ';">'+ item.labelName +'</span>')
                })
                this.contentArr = this.findLeafItem();
                ...
                if(data.children.length != 0 ) {
                    ...
                    if(!checked) {
                        ...
                        for(let i = 0; i < diffArr.length; i++) {
                            $('#content span[data-id = '+diffArr[i].uniqueId+']').remove();
